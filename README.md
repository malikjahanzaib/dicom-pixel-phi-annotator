...
5. **Detect text** (or `T`) runs a locally bundled OCR engine over the image's native raster and proposes a dashed box per detected text region. Click a suggestion — on the canvas, or in the **Detected text** panel where the read text is listed — to accept it as an ordinary zone.
   ...
   **Detection settings** shape the run. What suits one corpus suits another badly — a GE caption bar wants one padded box over the whole strip, a scattered overlay wants tight per-word boxes — so the shape of a detection is a setting rather than a constant:

   | Control | What it does |
   | --- | --- |
   | **Boxes** | Which level of the engine's own hierarchy becomes a box: one per word, line (the default), paragraph, or block. Coarser levels give fewer, larger boxes. |
   | **Padding** | How far each box grows beyond the glyph bounds, 0–40 px. Four is the default; tight bounds clip glyph edges and a clipped box uncovers text. Padding always clamps to the raster. |
   | **Merge gap** | Joins detections whose gap is within the tolerance into one box, so words on one caption line become one redaction. Off by default. The floor is applied first, so speckle below it can never drag a real box outwards. |
   | **Confidence floor** | The 0–100% threshold described above. |
   | **Min width / Min height** | Discards boxes smaller than this, which is what most speckle misread as text comes out as. |
   | **Min chars** | How many letters or digits a region needs to count, ignoring punctuation. Two by default — a lone glyph is the commonest thing texture is misread as. |
   | **Upscale** | Enlarges the raster 2–4× *before* the engine reads it, which recovers small burned-in text the engine otherwise skips at native size. Boxes are still reported in source-raster pixels: the factor is divided back out. |

   Every control but **Upscale** reshapes output the engine has already produced, so moving one re-derives in place — instantly, with no second run. Upscale changes what the engine is given, so it applies on the next **Detect text**. A settings change never revives a suggestion already accepted or dismissed. Settings persist with the workspace, and a **profile** names a full set of them so a corpus's settings can be restored in one selection instead of six. Profiles shape suggestions only: no zone, template or export is affected by one.
...
- OCR with the network blocked: the engine, its core and its language data loading from disk, line suggestions padded and clamped inside the raster, acceptance only on request, and no completeness claim in any status string. Detection settings re-deriving one run in place — words splitting lines, a merge gap rejoining them, padding still clamped to the raster — and a profile saving, restoring and deleting a full set of them.
...
- `src/ocr.js`: pure detection geometry — granularity, padding, clamping, size and character floors, merging, profiles, and conservative coverage. Everything here derives from a cached engine run; nothing in it calls the engine.
