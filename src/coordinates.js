...
export function buildPipelineExport(images) {
  const annotations = {};
  for (const image of images) {
    ...
    for (const value of zones) {
      ...
      if (!zone) continue;
      const combo = annotations[image.combo] ||= {};
      const group = combo[`${image.width}x${image.height}`] ||= { ref_width: image.width, ref_height: image.height, zones: [] };
      // The pipeline consumes layout zones: union across samples AND frames, never a frame index.
      // Keep different notes, remove only exact duplicates.
      if (!group.zones.some(z => zoneKey(z) === zoneKey(zone))) group.zones.push(zone);
    }
  }
...
export function pipelineLayout(images, combo, width, height) {
  if (!/^\d+$/.test(combo || '')) return null;
  const zones = [], seen = new Set();
  for (const image of images) {
    if (image.combo !== combo || image.width !== width || image.height !== height) continue;
    for (const value of Object.values(image.frames).flat()) {
      const zone = exportZone(value, image);
      if (!zone || seen.has(zoneKey(zone))) continue;
      seen.add(zoneKey(zone)); zones.push(zone);
    }
  }
  return zones;
}
